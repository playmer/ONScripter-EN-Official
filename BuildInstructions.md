...
These are instructions for me when I come back to this branch. You need `Microsoft Visual Studio .NET 2003` in a folder. Next to that folder you need this script (adapted from the `vcvars32.bat` within that folder):

`vcvars32_2003.bat`
```batch
@SET VSINSTALLDIR=%~dp0\Microsoft Visual Studio .NET 2003\Common7\IDE
@SET VCINSTALLDIR=%~dp0\Microsoft Visual Studio .NET 2003
@SET FrameworkDir=C:\WINDOWS\Microsoft.NET\Framework
@SET FrameworkVersion=v1.1.4322
@SET FrameworkSDKDir=%~dp0\Microsoft Visual Studio .NET 2003\SDK\v1.1
...
Run that script, then you can cd into ONScripter-EN and run:
```batch
cmake --preset=ninja -B build_win9x
cmake --build build_win9x --config Release
```