...
std::string QtBasicWindow::Command_InputStr(std::string& display, int maximumInputLength, bool forceDoubleByte, const int* w, const int* h, const int* input_w, const int* input_h)
{
  return InputStrDialog::getInputStr(display, maximumInputLength, forceDoubleByte, NULL, NULL, NULL, NULL, NULL);
}
    ...
    default:
    {
      ...
      QObject::connect(action, &QAction::triggered, [this, function = input.m_function]() {
          switch (function)
          {
            ...
            {
              VolumeDialog::adjustVolumeSliders(m_onscripterLabel, m_onscripterLabel->voice_volume, m_onscripterLabel->se_volume, m_onscripterLabel->music_volume, NULL);
              break;
            }
            case MenuBarFunction::END:
            {
              std::string output = "Are you sure you want to quit?";
              if (ExitDialog::shouldExit(output, NULL))
              {
